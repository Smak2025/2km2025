...
<!--        </form>-->
        ...
        $stud_id = -1;
        try {
            if (isset($_GET['stud_id'])) $stud_id = (int)htmlspecialchars($_GET['stud_id']);
        } catch (Exception $e) {
        }
        $points = $this->helper->getPointsForStudent($stud_id);
        print '<form method="POST" action="db_test_2.php">';
        ...
        foreach ($points as $point) {
            ...
            print '<tr>';
            print '<td><input type="checkbox" name="row_'.$point['id'].'"></td>';
            foreach ($point as $k => $v) {
        ...
        print '<input type="submit" value="Delete Selected Points">';
        print '</form>';
    }
}
